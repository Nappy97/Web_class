				...
				<ul class="navbar-nav me-auto">
					...
					<sec:authorize access="isAnonymous()">
						<li class="nav-item" TODO=""><a class="nav-link"
							href="javascript:void(0)">Anonymous</a></li>
					</sec:authorize>

					<!-- 로그인한 사람에게만 보이는 메뉴 -->
					<sec:authorize access="isAuthenticated()">
						<li class="nav-item" TODO=""><a class="nav-link"
							href="javascript:void(0)">Authenticated</a></li>
					</sec:authorize>

					<!-- 로그인한 사람중 ROLE_MEMBER & ROLE_ADMIN 권한 있는 사람에게만 보이는 메뉴 -->
					<sec:authorize access="hasAnyRole('ADMIN', 'MEMBER')">
						<li class="nav-item" TODO=""><a class="nav-link"
							href="javascript:void(0)">Member</a></li>
					</sec:authorize>
				...
				<sec:authorize access="isAuthenticated()">
					<form action="${pageContext.request.contextPath}/user/logout"
						...
						<span class="d-flex"> <span class="text-light p-2"><span
								TODO="">${username } (${userdetails.user.username })</span> 님
								환영합니다</span> <span><button class="btn btn-danger" type="submit">Logout</button></span>
						</span>
					</form>
				</sec:authorize>
