...
//				.password("1234") // 나중에 Security 적용하면 PasswordEncoder사용해야함.
		...
		User user2 = User.builder().username("USER2")
				.password(passwordEncoder.encode("1234")).name("회원2").build();

		User admin1 = User.builder().username("ADMIN1").password(passwordEncoder.encode("1234")).name("관리자1").build();
		...
		// 댓글 Comment
		Comment c1 = Comment.builder().content("1. user1이 1번글에 댓글 작성.").user(user1) // 누가 작성
		...
		// 첨부파일
		FileDTO file1 = FileDTO.builder().file("face01.png").source("face01.png").write(w1.getId()).build();
		FileDTO file2 = FileDTO.builder().file("face02.png").source("face02.png").write(w1.getId()).build();
		FileDTO file3 = FileDTO.builder().file("face03.png").source("face03.png").write(w2.getId()).build();
		FileDTO file4 = FileDTO.builder().file("face04.png").source("face04.png").write(w2.getId()).build();
		FileDTO file5 = FileDTO.builder().file("face05.png").source("face05.png").write(w3.getId()).build();
		FileDTO file6 = FileDTO.builder().file("face06.png").source("face06.png").write(w3.getId()).build();
		FileDTO file7 = FileDTO.builder().file("face07.png").source("face07.png").write(w4.getId()).build();
		FileDTO file8 = FileDTO.builder().file("face08.png").source("face08.png").write(w4.getId()).build();
